...
document.getElementById("addRow").addEventListener("click", () => {
  ...
  tr.innerHTML = `
    ...
    <td>0.00</td>
    <td><button class="btn ghost remove">Del</button></td>
  `;
...
document.getElementById("exportPng").addEventListener("click", () => {
  html2canvas(document.querySelector("#invoiceArea"), { scale: 2 }).then(
    (canvas) => {
...
document.getElementById("downloadPdf").addEventListener("click", () => {
  html2canvas(document.getElementById("invoiceArea"), { scale: 2 }).then(
    (canvas) => {
...
document.getElementById("togglePaid").addEventListener("click", () => {
  ...
  if (st.classList.contains("paid")) {
    ...
    st.classList.add("unpaid");
    st.innerText = "NOT PAID";
  } else {
    st.classList.remove("unpaid");
...
document.getElementById("saveInvoice").addEventListener("click", () => {
  const rows = [...document.querySelectorAll("#itemsTable tbody tr")].map(
    (tr) => ({
  ...
  const invoice = {
    ...
    shipping_address: document.querySelector(".card:nth-child(2) div")
      .innerText,
    items: rows,
    ...
    status: document.getElementById("stamp").classList.contains("paid")
      ? "PAID"
      : "NOT PAID",
  };
...
window.onload = () => {
  ...
  document.querySelector(".card:nth-child(1) div").innerText =
    inv.billing_address;
  document.querySelector(".card:nth-child(2) div").innerText =
